...
increaseButton.addEventListener('click', () => {
	...
	for (const selection of selections) {
		...
		for (const decoration of decorations) {
			...
			const newFontSize = parseFloat((parseFloat(options.fontSize) + increment).toFixed(1));
			const newLineHeight = options.lineHeight + increment;
			const newLineHeightFixed = Math.round(newLineHeight * 100) / 100;

			console.log('newFontSize: ', newFontSize);
			console.log('newLineHeight: ', newLineHeightFixed);
